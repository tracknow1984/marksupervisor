...
.side.smartNav{background:linear-gradient(180deg,#0a111b 0%,#0c1522 100%)}
...
(()=>{
  ...
  function renderResults(){const q=(search.value||'').trim().toLowerCase(),pool=q?[...actions,...destinations]:actions;resultRows=pool.filter(x=>!q||((x.label+' '+x.desc).toLowerCase().includes(q))).slice(0,12);selected=Math.min(selected,Math.max(0,resultRows.length-1));results.innerHTML=resultRows.length?resultRows.map(resultHtml).join(''):'<div style="padding:24px;text-align:center;color:#8793a3;font-size:12px">No matching pages or actions.</div>'}
