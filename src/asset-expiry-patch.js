...
(()=>{
  ...
  function decorate(){ensureHeader();const byId=new Map(assets.map(a=>[String(a.id),a]));rows.querySelectorAll('tr').forEach(tr=>{const view=tr.querySelector('[data-view]');if(!view)return;const asset=byId.get(String(view.dataset.view));if(!asset)return;let cell=tr.querySelector('.assetExpiryCell');if(!cell){cell=document.createElement('td');cell.className='assetExpiryCell';tr.insertBefore(cell,tr.lastElementChild||null)}cell.innerHTML=lightButton(asset)});rows.querySelectorAll('[data-expiry-id]').forEach(b=>b.onclick=e=>{e.stopPropagation();openExpiry(b.dataset.expiryId)})}
