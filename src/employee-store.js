...
function initialise(target){if(process.env.SV365_TENANT_ID)return target;if(!Array.isArray(target))throw new Error('Employee store target must be an array');const saved=read().employees;if(saved.length)target.splice(0,target.length,...saved);else write({version:1,employees:target});return target}
function saveAll(target){if(process.env.SV365_TENANT_ID)return target;if(!Array.isArray(target))throw new Error('Employees must be an array');write({version:1,employees:target});return target}
