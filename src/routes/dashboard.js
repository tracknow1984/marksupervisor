...
<div class="dash">
<section id="clientBrand" aria-label="Client" style="background:#fff;border:1px solid #e5eaf0;border-radius:16px;padding:20px 26px;margin-bottom:18px;display:none;align-items:center;gap:20px;flex-wrap:wrap"><img id="clientBrandLogo" alt="" hidden style="max-width:100%;max-height:100px;width:auto;object-fit:contain"><strong id="clientBrandName" style="font-size:22px;color:#15283c;overflow-wrap:anywhere"></strong></section>
<script>(()=>{fetch('/api/auth/me',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('Account unavailable');return r.json()}).then(d=>{const c=d.company;if(!c)return;document.getElementById('clientBrandName').textContent=c.name;const img=document.getElementById('clientBrandLogo');if(c.logo){img.src=c.logo;img.alt=c.name+' logo';img.hidden=false;img.onerror=()=>{img.hidden=true}}document.getElementById('clientBrand').style.display='flex'}).catch(()=>{})})();</script>
