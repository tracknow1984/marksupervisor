...
<div class="dash">
<section id="clientBrand" aria-label="Client logo" style="background:#fff;border:1px solid #e5eaf0;border-radius:16px;padding:16px 24px;margin-bottom:18px;display:none;align-items:center"><img id="clientBrandLogo" alt="Client logo" style="display:block;width:280px;max-width:100%;height:72px;object-fit:contain;object-position:left center"></section>
<script>(()=>{fetch('/api/auth/me',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('Account unavailable');return r.json()}).then(d=>{const c=d.company;if(!c?.logo)return;const brand=document.getElementById('clientBrand'),img=document.getElementById('clientBrandLogo');img.onload=()=>{brand.style.display='flex'};img.onerror=()=>{brand.style.display='none'};img.src=c.logo}).catch(()=>{})})();</script>
