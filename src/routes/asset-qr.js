...
router.get('/assets/:id',(req,res)=>{
  ...
  res.send(page('assets',a.name,`<section class="title"><div><h1>${esc(a.name)}</h1><p>${esc(a.type)} · ${esc(a.rego)}${a.plantId?' · '+esc(a.plantId):''}</p></div><div class="actionlinks"><a class="secondary" href="/assets">← Assets</a><a class="primary" href="/assets/${encodeURIComponent(a.id)}/qr">QR Code</a></div></section><section class="panel"><div class="viewerbody"><div class="detailgrid">${d('Asset Type',a.type)}${d('Registration',a.rego)}${d('Registration State',a.registrationState)}${d('Registration Expiry',a.registrationExpiry)}${d('Status',a.status)}${d('Make',a.make)}${d('Model',a.model)}${d('VIN',a.vin)}${d('Current Odometer',Number(a.reading||0).toLocaleString('en-AU'))}${d('Acquisition Date',a.acquisitionDate)}${d('Category',a.category)}${d('Refrigeration Inspection',a.refrigerationInspectionDate)}${d('Handbrake Alarm',a.handbrakeAlarm?'Yes':'No')}${d('Reverse Camera',a.reverseCamera?'Yes':'No')}${d('ADAS 4-Way Video',a.adasVideo?'Yes':'No')}${d('Insurance Expiry',a.insuranceExpiry)}${d('Compliance Plate Date',a.compliancePlateDate)}${d('COI Number',a.coiNumber)}${d('COI Due Date',a.coiDueDate)}${d('Asset / Plant ID',a.plantId)}${d('Location',a.location)}</div><h3>Additional Notes</h3><p>${esc(a.additionalNotes||'—')}</p></div></section>`));
});
...
router.get('/assets/:id/qr.svg',async(req,res)=>{
  const a=findAsset(req.params.id);if(!a)return res.status(404).send('Asset not found');
  try{
    let svg=await QRCode.toString(assetUrl(req,a),{type:'svg',errorCorrectionLevel:'M',margin:2});
    svg=svg.replace(/<svg([^>]*)width="[^"]+"([^>]*)height="[^"]+"([^>]*)>/, '<svg$1width="65mm"$2height="65mm"$3>');
    if(!/width="65mm"/.test(svg))svg=svg.replace('<svg ','<svg width="65mm" height="65mm" ');
    res.setHeader('Content-Type','image/svg+xml');
    if(req.query.download)res.setHeader('Content-Disposition',`attachment; filename="${String(a.rego||a.id).replace(/[^a-z0-9_-]/gi,'_')}-QR-65mm.svg"`);
    res.send(svg);
  }catch(e){res.status(500).send('Unable to generate QR code')}
});

router.get('/assets/:id/qr.png',async(req,res)=>{
  const a=findAsset(req.params.id);if(!a)return res.status(404).send('Asset not found');
  try{
    const png=await QRCode.toBuffer(assetUrl(req,a),{type:'png',errorCorrectionLevel:'M',margin:4,width:768});
    res.setHeader('Content-Type','image/png');
    if(req.query.download)res.setHeader('Content-Disposition',`attachment; filename="${String(a.rego||a.id).replace(/[^a-z0-9_-]/gi,'_')}-QR-768px.png"`);
    res.send(png);
  }catch(e){res.status(500).send('Unable to generate QR code')}
});
