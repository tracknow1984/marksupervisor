...
function byToken(token){for(const doc of list()){const recipient=(doc.recipients||[]).find(x=>String(x.token)===String(token));if(recipient)return{document:{...doc,recipients:[recipient]},recipient}}return null}
